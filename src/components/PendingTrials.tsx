import { Link } from "react-router-dom";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Clock, Hourglass, X } from "lucide-react";
import { store, useStore, getAllListings } from "@/lib/store";
import { JoinRequest, SlotKey } from "@/lib/types";
import { fmtHour } from "@/lib/timeUtils";
import { toast } from "sonner";

function parseSlot(slot: SlotKey): { day: string; hour: number } {
  const [day, h] = String(slot).split("-");
  return { day, hour: parseInt(h, 10) };
}

interface Props {
  /** "learner" → show user's pending trial requests; "provider" → show pending trials for tutor's classes */
  side: "learner" | "provider";
}

export function PendingTrials({ side }: Props) {
  const learner = useStore((s) => s.learner);
  const requests = useStore((s) => s.requests);
  const listings = useStore((s) => s.listings);
  const allListings = getAllListings();

  let rows: { req: JoinRequest; listing: ReturnType<typeof allListings.find> }[] = [];

  if (side === "learner") {
    const myName = learner.name || "Guest";
    rows = requests
      .filter((r) => r.learnerName === myName && r.isTrial && r.status === "Pending")
      .map((r) => ({ req: r, listing: allListings.find((l) => l.id === r.listingId) }));
  } else {
    const myIds = new Set(listings.map((l) => l.id));
    rows = requests
      .filter((r) => myIds.has(r.listingId) && r.isTrial && r.status === "Pending")
      .map((r) => ({ req: r, listing: allListings.find((l) => l.id === r.listingId) }));
  }

  if (rows.length === 0) return null;

  return (
    <Card className="p-4 space-y-3 border-warning/30 bg-warning/5">
      <div className="flex items-center justify-between gap-2">
        <h2 className="font-semibold flex items-center gap-2 text-sm">
          <Hourglass className="h-4 w-4 text-warning" />
          Pending trial requests
          <Badge variant="secondary" className="text-[10px] h-5">{rows.length}</Badge>
        </h2>
      </div>
      <div className="space-y-2">
        {rows.map(({ req, listing }) => {
          const { day, hour } = parseSlot(req.slot);
          return (
            <div
              key={req.id}
              className="flex items-center gap-3 rounded-lg border bg-card p-3"
            >
              <div className="h-9 w-9 rounded-lg bg-warning/15 text-warning grid place-items-center shrink-0">
                <Clock className="h-4 w-4" />
              </div>
              <div className="flex-1 min-w-0">
                <Link
                  to={`/listing/${req.listingId}`}
                  className="text-sm font-medium truncate hover:text-primary block"
                >
                  {listing?.title || "Class"}
                </Link>
                <div className="text-[11px] text-muted-foreground truncate">
                  {listing?.providerName || "Tutor"}
                </div>
                <div className="text-[11px] text-muted-foreground mt-0.5">
                  {req.startDate ? `${req.startDate} · ` : ""}{day} · {fmtHour(hour)}
                </div>
              </div>
              <Button
                size="icon"
                variant="ghost"
                className="h-8 w-8 text-muted-foreground hover:text-destructive shrink-0"
                title="Remove this request"
                onClick={() => {
                  if (confirm(`Remove this pending trial request for "${listing?.title || "this class"}"?`)) {
                    store.removeRequest(req.id);
                    toast.success("Request removed");
                  }
                }}
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
          );
        })}
      </div>
    </Card>
  );
}
